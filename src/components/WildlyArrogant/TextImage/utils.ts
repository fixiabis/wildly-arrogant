...
export const AVAILABLE_TEXT_IMAGE_WIDTH =
  TEXT_IMAGE_WIDTH - TEXT_IMAGE_BORDER_WIDTH * 2;
...
export class TextPainter {
  ...
  applyThemeSetting() {
    const {
      themeSetting: { type: themeType, color: themeColor },
    } = this;
  ...
  getBreakedTextLinesWithStyleByFontSize(
    fontSize: number = 30
  ): TextLinesWithStyle {
    ...
    for (let textLine of textLines) {
      ...
      if (textLineWidthOnCanvas > AVAILABLE_TEXT_IMAGE_WIDTH) {
        ...
        for (let textChar of textLine) {
          const newTextLineWidthOnCanvas = canvasContext.measureText(
            newTextLine + textChar
          ).width;

          if (newTextLineWidthOnCanvas <= AVAILABLE_TEXT_IMAGE_WIDTH) {
            newTextLine += textChar;
            maxTextLineWidthOnCanvas = Math.max(
              newTextLineWidthOnCanvas,
              maxTextLineWidthOnCanvas
            );
          } else {
    ...
    const paddingTop =
      (TEXT_IMAGE_HEIGHT - breakedTextLines.length * lineHeight) / 2 + 2.5;
  ...
  paintTextByTextLinesAndStyle({ textLines, ...style }: TextLinesWithStyle) {
    ...
    for (let textLine of textLines) {
      canvasContext.fillText(
        textLine,
        textLineStartPosition.x,
        textLineStartPosition.y
      );
  ...
  paintText() {
    ...
    for (let fontSize = 25; fontSize <= 60; fontSize += 5) {
      const breakedTextLinesWithStyle = this.getBreakedTextLinesWithStyleByFontSize(
        fontSize
      );
      const breakedTextLines = breakedTextLinesWithStyle.textLines;

      if (
        breakedTextLinesWithStyle.paddingTop < fontSize ||
        textLines.length < breakedTextLines.length
      ) {
        const breakedTextLinesWithStyle = this.getBreakedTextLinesWithStyleByFontSize(
          fontSize - 5
        );
