...
const GOOGLE_MARCO_URL =
  'https://script.google.com/macros/s/AKfycbxf6RbMgUDvM82LudhJLUdaogyp5yKVVp6rMu8SyE0IE3rWQcej/exec';

const App = () => {
  const [messageThemeType, setMessageThemeType] = useState<
    WildlyArrogant.ThemeType
  >(0);
  const [messageThemeIndex, setMessageThemeIndex] = useState(0);
  const [messageImageUrl, setMessageImageUrl] = useState('');
  const [
    { facebookPostId, isCreatingPost, messageText },
    setPostState,
  ] = useState({ facebookPostId: '', isCreatingPost: false, messageText: '' });
  const setIsCreatingPost = (isCreatingPost: boolean) =>
    setPostState({ facebookPostId, isCreatingPost, messageText });
  ...
  const chooseThemeColorRef = useRef<HTMLUListElement>() as RefObject<
    HTMLUListElement
  >;
  const chooseThemeTypeRef = useRef<HTMLUListElement>() as RefObject<
    HTMLUListElement
  >;
  ...
  if (isCreatingPost) {
    ...
    return (
      <div style={style}>
        <div id="loading"></div>
        <div className="blink" style={{ marginTop: 10 }}>
          正在建立貼文...
        </div>
      </div>
    );
  ...
  return (
    <>
      ...
      <main>
        <textarea
          value={messageText}
          placeholder="你最近最狂的事情是什麼?"
          onChange={handleTextareaChange}
        />

        <ul
          id="choose-theme-color"
          ref={chooseThemeColorRef}
          className="choose"
        >
          {WildlyArrogant.themeNames.map((themeName, themeIndex) => {
            ...
            return (
              <li key={themeIndex} style={style} onClick={handleClick}>
                {themeName}
              </li>
            );
          })}
        ...
        <ul id="choose-theme-type" ref={chooseThemeTypeRef} className="choose">
          {WildlyArrogant.ThemeTypeNames.map((themeTypeName, themeType) => {
            ...
            return (
              <li key={themeType} style={style} onClick={handleClick}>
                {themeTypeName}
              </li>
            );
          })}
      ...
      <footer>
        ...
        <a
          href="https://fb.me/wildly.arrogant"
          target="_blank"
